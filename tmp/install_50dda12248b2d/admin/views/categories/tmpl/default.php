...
if ( $this->lists['config']->show_category_color ) {
	$show_color = TRUE;
	$colspan = 5; 
} else {
	$colspan = 4;
	...
	<div id="editcell">
		...
		<thead>
			<tr>
				...
				<th>
					<?php echo JHTML::_('grid.sort',  'categoryname', 'a.categoryName', $this->lists['order_Dir'], $this->lists['order'] ); ?>
				</th>
				<?php if ( $show_color ): ?>
				<th>
		...
		for ($i=0, $n=count( $this->items ); $i < $n; $i++) {
			...
			$checked 	= JHTML::_('grid.id',   $i, $row->categoryID );
			$link 		= JRoute::_( 'index.php?option=com_simplecalendar&controller=category&task=edit&cid[]='. $row->categoryID );
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<td>
					<a href="<?php echo $link; ?>"><?php echo $row->categoryName; ?></a>
				</td>
				<?php if ( $show_color ): ?>
				<td>
					<span style="color: #<?php echo $row->category_color; ?>"><?php echo $row->category_color; ?></span>
				</td>
				<?php endif; ?>
				<td class="order">
